...
function(input, output, session) {
  ...
  observeEvent(eventExpr = input$test,
               handlerExpr = {
                 output$download_NEON_general <- downloadHandler(filename = "testt.zip",
                 content = function(file) {
                   setwd(paste0("/home/danielslee/NEON/filesToStack00004/"))
                   zip(zipfile = file, files = "NEON_STER_00004")
                   setwd('/srv/shiny-server/NEON-Hosted-Browser')
                 },
